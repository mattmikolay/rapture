...
data class RSequence(val entries: List<RObject> = emptyList()) : RObject("sequence") {
    override fun plus(other: RObject) = when (other) {
        is RSequence -> RSequence(entries + other.entries)
        else -> throw RapiraInvalidOperationError(Operation.Addition, other)
    }

    override fun times(other: RObject) = when (other) {
        is RInteger -> RSequence(arrayOfNulls<RObject>(other.value).flatMap { entries })
    ...
    override fun slice(start: RObject?, end: RObject?): RObject {
        ...
        return entries.subList(startIndex.value - 1, endIndex.value).toRSequence()
    }
